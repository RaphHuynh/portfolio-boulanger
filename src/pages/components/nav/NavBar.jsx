    ...
    return (
        <nav className="fixed w-full flex items-center px-20 py-2">
            ...
            <div className="flex mr-0 ml-auto">
                {link.map((item) => (
                    <a key={item.id} className="transition delay-150 box-border px-5 py-2 mr-5 border-2 rounded-full border-amber-500 text-center text-amber-500 text-lg bg-zinc-50 hover:bg-amber-500 hover:text-zinc-50" href={item.id}>{item.text}</a>
                ))}
            </div>
        </nav>
    )
